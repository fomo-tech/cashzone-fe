...
const MobileNavigation = () => {
  ...
  const getLinkClasses = (path: string) => {
    ...
    if (isActive(path)) {
      return `${baseClasses} text-orange-600 font-bold`;
    }

    return `${baseClasses} text-slate-600 hover:text-orange-600 hover:bg-gradient-to-r hover:from-pink-50/50 hover:to-orange-50/50 font-bold`;
  };
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white/98 backdrop-blur-sm border-t border-slate-100 z-50 md:hidden pb-safe">
        <div className="grid grid-cols-5 gap-1 px-1 py-2">
          ...
          <Link to="/ranks" className={getLinkClasses("/ranks")}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
